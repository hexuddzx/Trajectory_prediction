...
hiddenSize = 10
r = nn.Recurrent(
    hiddenSize, nn.Linear(7,hiddenSize),
...
trainLogger = optim.Logger(paths.concat('/home/hexu/mobile-data/logs', '7_24time_rnntrain.log'))
testLogger = optim.Logger(paths.concat('/home/hexu/mobile-data/logs', '7_24time_rnntest.log'))
...
-- 在最后一个迭代期将预测数据与实际数据写入文件以便观察
      if flag == 30 then
        local file = io.open('/home/hexu/mobile-data/result/7_24time_rnntargets.txt','a+')
        local file2 = io.open('/home/hexu/mobile-data/result/7_24time_rnnpreds.txt','a+')
        for i = 1, batchSize do
...
torch.save('/home/hexu/mobile-data/new_data/model/7_24time_rnnmodel.lua',model)
