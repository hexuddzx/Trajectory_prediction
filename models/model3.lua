...
model = torch.load('/home/hexu/mobile-data/new_data/model/model.lua')
...
train_dataset = fileio('/home/hexu/mobile-data/new_data/multiple/1_trainset6.txt',3000000,8)  
test_dataset = fileio('/home/hexu/mobile-data/new_data/multiple/1_testset6.txt',900000,8) 
...
trainLogger = optim.Logger(paths.concat('/usr/local/trajectory-prediction/logs', '1train6.log'))
testLogger = optim.Logger(paths.concat('/usr/local/trajectory-prediction/logs', '1test6.log'))
...
-- 在最后一个迭代期将预测数据与实际数据写入文件以便观察
      if flag == 30 then
        local file = io.open('/home/hexu/mobile-data/new_data/result/targets6.txt','a+')
        local file2 = io.open('/home/hexu/mobile-data/new_data/result/preds6.txt','a+')
        for i = 1, batchSize do
